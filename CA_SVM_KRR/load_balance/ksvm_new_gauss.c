...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    /*
    char filename[50] = "rcv1_train.binary";

    int N = 20242;   // n is num observation, m is num features
    int M = 47236;   // n,m are dimension of the A, dimension of b is n * 1

    // define hyper parameters: BLKSIZE, LAMBDA, MAXIT, GAMMA, KERNEL, tol, seed, s (if cabdccd)
    char KERNEL[10] = "linear";
    int MAXIT = atoi(argv[1]);
    double GAMMA = 1;
    int seed = 42;
    double LAMBDA = 1;  
    ...
    */
    ...
    // Warmup phase: simple point-to-point communication
    for (int i = 0; i < 16; ++i) {
        if (rank == 0) {
            int number;
    ...
    int *loadPerRank = (int *) calloc(size, sizeof(int));
    int *featuresPerRank = (int *) calloc(size, sizeof(int));
    // int *assignedFeatures = (int *) calloc(M, sizeof(int));
    int *featureNnz = (int *) calloc(M, sizeof(int));
    // int *currentFeatureIndex = (int *) calloc(size, sizeof(int));
    
    if (strcmp(filename, "news20.binary") == 0) {
        // Read nnz for each feature
        read_feature_nnz("news_nnz_output.txt", M, featureNnz);

        // First loop: Determine the number of features per rank
        // add feature to the least current nnz rank
        for (int i = 0; i < M; i++) {
            int minLoadRank = 0;
            for (int j = 1; j < size; j++) {
                if (loadPerRank[j] < loadPerRank[minLoadRank]) {
                    minLoadRank = j;
                }
            }
            loadPerRank[minLoadRank] += featureNnz[i]; // no need for csr_setup, we can setup csr matrix with this information
            featuresPerRank[minLoadRank]++; // find the number of feature of current rank
        }


        // Allgather to distribute the featuresPerRank array
        // MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, featuresPerRank, 1, MPI_INT, MPI_COMM_WORLD);

        // Ensure all processes have the correct featuresPerRank before proceeding
        MPI_Barrier(MPI_COMM_WORLD);

        // ########################
        // The code above are fine
        // ########################
        
        // Reallocate feature_indices based on featuresPerRank[rank]
        if (feature_indices != NULL) {
            free(feature_indices);
        }
        feature_indices = (int *) calloc(featuresPerRank[rank], sizeof(int));        
        
        // Ensure that the memory allocation was successful
        if (feature_indices == NULL) {
            fprintf(stderr, "Memory allocation for feature_indices failed.\n");
            MPI_Abort(MPI_COMM_WORLD, 1); // Use MPI_Abort to handle allocation failure in MPI environment
        }
        
        memset(loadPerRank, 0, size * sizeof(int));
        //memset(feature_indices, 0, featuresPerRank[rank] * sizeof(int));
        // printf("Size of feature indices: %d\n", featuresPerRank[rank]);
        
        // record current indices of the feature_indices
        // This is rank dependent
        int curr_fi = 0;
        int sub_blk = featuresPerRank[rank];

        // Second loop: Assign feature indices to assignedFeatures for each rank
        for (int i = 0; i < M; i++) {
            int minLoadRank = 0;
            for (int j = 1; j < size; j++) {
                if (loadPerRank[j] < loadPerRank[minLoadRank]) {
                    minLoadRank = j;
                    //printf("%d ", minLoadRank);
                }
            }
            loadPerRank[minLoadRank] += featureNnz[i];
            if (rank == minLoadRank) {
                feature_indices[curr_fi] = i;
                curr_fi++;
                //printf("%d ", i);
            }
        }
        ...
        // Print the total nnz assigned to each process
        
        /*
        for (int i = 0; i < size; i++) {
            if (rank == i) {
                printf("Rank %d has a total nnz of %d\n", rank, loadPerRank[rank]);
                //for (int j = 0; j < featuresPerRank[rank]; j++) {
                for (int j = 0; j < 10; j++) {
                    //if (feature_indices[j] == 0) {
                    //    continue;
                    //}
                    printf("%d ", feature_indices[j]);
                }                
            }
            MPI_Barrier(MPI_COMM_WORLD); // Ensure orderly printing
        }
        */
    }
    
    if (rank == 0) {
        printf("Feature Partition finished   |    %s\n", filename);
    }
    ...
    csr_setup_bdcd(csr, max_line, filename, N, feature_indices, featuresPerRank[rank]);
    ...
    if (csr->nnz > 0) {
        read_data_sparse_bdcd(csr, scale_A, max_line, filename, featuresPerRank[rank], feature_indices);
    }
    ...
    double * idx_norm = (double *) calloc(BLKSIZE, sizeof(double));
    ...
    while (iter < MAXIT) {
        ...
        if (scale_A->nnz > 0) {
            sparse_sample_A_bdcd(scale_sample, scale_A, 1, featuresPerRank[rank], curr_idx);

            idx_norm[0] = norm[curr_idx];
        }
        ...
        if (scale_sample->nnz > 0 && scale_A->nnz > 0) {
            if (strcmp(KERNEL, "linear") == 0) {
                linear(scale_sample, scale_A, N, featuresPerRank[rank], 1, k_para);   // as return a pointer
            }
            else if (strcmp(KERNEL, "poly") == 0) {
                linear(scale_sample, scale_A, N, featuresPerRank[rank], 1, k_para); 
            }
            else if (strcmp(KERNEL, "gauss") == 0) {
                gaussian(1, N, featuresPerRank[rank], scale_sample, scale_A, k_para, curr_idx, norm, &kernel_in_gauss, &kernel_sp2md, ones_N, ones_blksize, idx_norm, &handle_time_out); 
            }
        }
        
        // add the barrier to it only keep working after all kernel computation ended
        MPI_Barrier(MPI_COMM_WORLD);
        ...
        if (strcmp(KERNEL, "poly") == 0) {
            ...
            cblas_dscal(N, GAMMA, k_row, 1);
        }
        else if (strcmp(KERNEL, "gauss") == 0) {
             // Apply the exponential function element-wise to the k array
            cblas_dscal(N, GAMMA*(-1.0), k_row, 1);
            for (int i = 0; i < N; i++) {
                k_row[i] = exp(k_row[i]);
            }
        }
    ...
    double max_major_time = 0.0;
    double min_major_time = 0.0;

    double avg_kernel_computation = 0.0;   
    MPI_Allreduce(&kernel_computation, &avg_kernel_computation, 1,  MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    avg_kernel_computation = avg_kernel_computation / (size * 1.0);
    double avg_allreduce = 0.0;   
    MPI_Allreduce(&allreduce_time, &avg_allreduce, 1,  MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    avg_allreduce = avg_allreduce / (size * 1.0);
    if (rank == 0) {
    printf("avg_allreduce: %.5f, avg_kernel_comp: %.5f\n", avg_allreduce, avg_kernel_computation);
    }
    // Find the maximum kernel_computation time among all processes
    double max_kernel_computation = 0.0;
    MPI_Reduce(&kernel_computation, &max_kernel_computation, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    // find the minimum kernel computation time
    double min_kernel_computation = 0.0;
    MPI_Reduce(&kernel_computation, &min_kernel_computation, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        fp = fopen("caksvm_data_news_gauss_balance.csv", "a");
        if (fp == NULL) {
        ...
        major_time = (avg_kernel_computation + avg_allreduce + sample_time + gradient_comp + alpha_updating + memory_reset) / (MAXIT * 1.0);
        max_major_time = (max_kernel_computation + avg_allreduce + sample_time + gradient_comp + alpha_updating + memory_reset) / (MAXIT * 1.0);
        min_major_time = (min_kernel_computation + avg_allreduce + sample_time + gradient_comp + alpha_updating + memory_reset) / (MAXIT * 1.0);
        
        // write some data rows to the CSV file
        // major time is the time for 6 major piece of runtime component, the part we care most
        // fprintf(fp, "%s, %s, %d, %d, %d, %d, %d, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f\n", filename, KERNEL, size, s, BLKSIZE, degree, MAXIT, GAMMA, LAMBDA, csr_setup_time, avg_kernel_computation / (MAXIT * 1.0), avg_allreduce / (MAXIT * 1.0), sample_time / (MAXIT * 1.0), csr_read_time, gradient_comp / (MAXIT * 1.0), alpha_updating / (MAXIT * 1.0), memory_reset / (MAXIT * 1.0), major_time,runtime);
        fprintf(fp, "%s, %s, %d, %d, %d, %d, %d, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f\n", filename, KERNEL, size, s, BLKSIZE, degree, MAXIT, GAMMA, LAMBDA, csr_setup_time, max_kernel_computation / (MAXIT * 1.0), min_kernel_computation / (MAXIT * 1.0), avg_kernel_computation / (MAXIT * 1.0), avg_allreduce / (MAXIT * 1.0), sample_time / (MAXIT * 1.0), csr_read_time, gradient_comp / (MAXIT * 1.0), alpha_updating / (MAXIT * 1.0), memory_reset / (MAXIT * 1.0), max_major_time, min_major_time,major_time,runtime);
        // close the CSV file
        fclose(fp);

        // print the result
        //printf("%s, %s, %d, %d, %d, %d, %d, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f, %.10f\n", filename, KERNEL, size, s, BLKSIZE, degree, MAXIT, GAMMA, LAMBDA, csr_setup_time, avg_kernel_computation / (MAXIT * 1.0), avg_allreduce / (MAXIT * 1.0), sample_time / (MAXIT * 1.0), csr_read_time, gradient_comp / (MAXIT * 1.0), alpha_updating / (MAXIT * 1.0), memory_reset / (MAXIT * 1.0), major_time,runtime);
        //printf("%.10f, %.10f, %.10f, %.10f, %.10f\n", avg_kernel_computation, kernel_loop_time / (MAXIT * 1.0), kernel_in_gauss / (MAXIT * 1.0), kernel_sp2md / (MAXIT * 1.0), handle_time_out / (MAXIT * 1.0));
    }
    ...
    free(loadPerRank);
    free(featuresPerRank);
    //free(assignedFeatures);
    free(featureNnz);
    // free(currentFeatureIndex);
...
void gaussian(int blksize, int n, int m, Element *sample_A_sparse, Element * csr, double *k, int idx, double * norm) {
    ...
    // we already have the -2*K, so we only need to do the sum norm & norm
...
*/
void gaussian(int blksize, int n, int m, Element *sample_A_sparse, Element * csr, double *k, double gamma, int idx, double * norm, double * gauss_in_loop, double * kernel_sp2md, double * ones_N, double * ones_blksize, double * idx_norm, double * handle_time_out) {

    
    // computes the ai ^T aj matrix, which is a [blksize * s, N] matrix
    double handle_time = 0.0;
    handle_time = MPI_Wtime();
    // Create the handle for the CSR matrix
    sparse_matrix_t sample_handle;
    mkl_sparse_d_create_csr(&sample_handle, SPARSE_INDEX_BASE_ZERO, blksize, m, sample_A_sparse->row_b, sample_A_sparse->row_e, sample_A_sparse->col, sample_A_sparse->val);
    // Descriptor of main sparse matrix properties                              
    ...
    descr_Rows.mode = SPARSE_FILL_MODE_FULL;
    descr_Rows.diag = SPARSE_DIAG_NON_UNIT;
    handle_time_out[0] = handle_time_out[0] + MPI_Wtime() - handle_time;
...
// Function to read nnz for each feature from a file
void read_feature_nnz(const char *filename, int numFeatures, int *featureNnz) {
