...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    if (csr->nnz > 0) {
        read_data_sparse_bdcd(csr, scale_A, max_line, filename, feature_indices, featuresPerRank[rank]);
    }
    ...
    if (csr->nnz > 0) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            // TODO: use the cblas routines to do the work
            // val = cblas_dnrm2(elts, &csr->val[row_b[i]], 1);
    ...
    while (iter < MAXIT) {
        ...
        if (scale_A->nnz > 0) {
            sparse_sample_A_bdcd(scale_sample, scale_A, 1*s, featuresPerRank[rank], idx);
        ...
        if (scale_sample->nnz > 0 && scale_A->nnz > 0) {
            if (strcmp(KERNEL, "linear") == 0) {
                linear(scale_sample, scale_A, N, featuresPerRank[rank], 1*s, k_para);   // as return a pointer
            }
            else if (strcmp(KERNEL, "poly") == 0) {
                linear(scale_sample, scale_A, N, featuresPerRank[rank], 1*s, k_para); 
            }
            else if (strcmp(KERNEL, "gauss") == 0) {
                gaussian(1*s, N, featuresPerRank[rank], scale_sample, scale_A, k_para, idx, norm, &kernel_in_gauss, &kernel_sp2md, ones_N, ones_blksize, idx_norm, &handle_time_out); 
            }
        }
    ...
    // MPI_Reduce(&kernel_computation, &max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        //printf(s"%.10f, %.10f, %.10f, %.10f, %.10f\n", kernel_computation / (MAXIT * 1.0), kernel_loop_time / (MAXIT * 1.0), kernel_in_gauss / (MAXIT * 1.0), kernel_sp2md / (MAXIT * 1.0), handle_time_out / (MAXIT * 1.0));
    }
