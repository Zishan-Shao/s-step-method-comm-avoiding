...
# Set file path
file_path = '/Users/shaozishan/Desktop/new_outputs/final_plots/Data/para_ca_data_colon_gauss.csv'
...
df = df[df['num_process'].isin([2,4,8,16,32,64,128])]
df = df[df['blksize'].isin([blksize])]
...
if "news" in file:
    filename = f'Cabdcd_Newsbinary_{kernel_type}_b{blksize}_Max.pdf'
    DName = "newsbinary"
elif "duke" in file:
    filename = f'Cabdcd_Duke_{kernel_type}_b{blksize}_Max.pdf'
    DName = "duke"
elif "colon" in file:
    filename = f'Cabdcd_Colon_{kernel_type}_b{blksize}_Max.pdf'
    DName = "colon"
elif "leu" in file:
    filename = f'Cabdcd_Leu_{kernel_type}_b{blksize}_Max.pdf'
    DName = "leu"
elif "rcv" in file:
    filename = f'Cabdcd_rcv_{kernel_type}_b{blksize}_Max.pdf'
    DName = "rcv"
elif "mock" in file:
    filename = f'Cabdcd_synthetic_{kernel_type}_b{blksize}_Max.pdf'
    DName = "mock"
elif "improved" in file:
    filename = f'Cabdcd_synthetic_{kernel_type}_b{blksize}_Max_Improved.pdf'
    DName = "improved"
...
ax2.plot(adjusted_num_processes, min_runtime_s, color='r', marker='o', label='s-step BDCD Runtime', linewidth=3)
ax2.plot(adjusted_num_processes, major_time_s_0, color='g', marker='x', linestyle='--', label='BDCD runtime', linewidth=3)
...
plot_title = f'BDCD Strong Scaling (Max) | {DName}, {kernel_type}, b = {blksize}'
...
directory = '/Users/shaozishan/Desktop/new_outputs/final_plots/HPCAsia/Ridge/general_plot/colon'
