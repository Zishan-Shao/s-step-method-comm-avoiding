...
# Load the dataset
df = pd.read_csv('/Users/shaozishan/Desktop/new_outputs/load_balance/prefinal/caksvm_data_mock2_gauss.csv')
...
if "news" in file:
    filename = 'Caksvm_Newsbinary_{}_Max.pdf'.format(kernel_type)
if "duke" in file:
    filename = 'Caksvm_Duke_{}_Max.pdf'.format(kernel_type)
if "colon" in file:
    filename = 'Caksvm_Colon_{}_Max.pdf'.format(kernel_type)
if "leu" in file:
    filename = 'Caksvm_Leu_{}_Max.pdf'.format(kernel_type)
if "rcv" in file:
    filename = 'Caksvm_rcv_{}_Max.pdf'.format(kernel_type)
if "mock" in file:
    filename = 'Caksvm_mock_{}_Max.pdf'.format(kernel_type)
if "improved" in file:
    filename = 'Caksvm_mock_{}_Max_Improved.pdf'.format(kernel_type)
...
for i, txt in enumerate(speedup_s_0):
    ax1.annotate(f'{{txt:.2f}}x'.format(txt=txt), (adjusted_num_processes[i], speedup_s_0[i]), textcoords="offset points", xytext=(0,10), ha='center')
...
# Plot line for max_major_time at s=0
max_major_time_s_0 = [df[(df['num_process'] == num_process) & (df['s'] == 0)]['max_major_time'].iloc[0] for num_process in unique_num_processes]
ax2.plot(adjusted_num_processes, max_major_time_s_0, color='g', marker='x', linestyle='--', label='DCD runtime')
...
# Show the plot
plt.title('Dual Coordinate Descent Strong Scaling (Max) | Synthetic, Gauss')
