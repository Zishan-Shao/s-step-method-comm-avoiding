...
# Load the dataset
# df = pd.read_csv('/Users/shaozishan/Desktop/new_outputs/modified/caksvm_data_duke_gauss.csv')
df = pd.read_csv('/Users/shaozishan/Desktop/new_outputs/caksvm_data_newsbinary_gauss.csv')

# Filter the DataFrame by specific 'num_process' values
df = df[df['num_process'].isin([32,64,128,256,512,1024,2048,4096])] #8,16,32,64,128,
df = df[df['s'].isin([0,2,4,8,16,32,128,256])]

# Determine the kernel type and filename based on the original file name
file = 'caksvm_data_newsbinary_gauss.csv'
if "newsbinary" in file:
    filename = 'Caksvm_Newsbinary_{}_General.png'.format("gauss")
if "duke" in file:
    filename = 'Caksvm_Duke_{}_General.png'.format("gauss")
if "colon" in file:
    filename = 'Caksvm_Colon_{}_General.png'.format("gauss")
if "leu" in file:
    filename = 'Caksvm_Leu_{}_General.png'.format("gauss")
...
ax1.bar(adjusted_num_processes, speedup_s_0, alpha=0.6, label='')
...
# Show the plot
plt.title('Dual Coordinate Descent Strong Scaling | Newsbinary')
